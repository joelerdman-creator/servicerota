...
function RequestChangeModal({ isOpen, onClose, event, role, sendRequestAction }) {
    ...
    const onSubmit = async (data)=>{
        ...
        try {
            ...
            // In a real app, you would get a list of eligible volunteers to email.
            // Here, we'll get the admin's email as the recipient.
            const church = db.getChurchSync();
...
function generateIcsFile(events) {
    let icsString = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//WeaversLoom//VolunteerScheduler//EN'
    ].join('\r\n');
    events.forEach((event)=>{
        event.roles.forEach((role)=>{
            ...
            const eventData = [
                'BEGIN:VEVENT',
                `UID:${event.id}-${role.id}@weaversloom.app`,
                `DTSTAMP:${formatToUTC(new Date())}`,
                `DTSTART:${formatToUTC(startDate)}`,
                `DTEND:${formatToUTC(endDate)}`,
                `SUMMARY:${role.roleName} - ${event.eventName}`,
                `DESCRIPTION:Your assigned role is ${role.roleName} for the event: ${event.eventName}.${event.notes ? `\\n\\nNotes: ${event.notes}` : ''}`,
                'END:VEVENT'
            ].join('\r\n');
            icsString += '\r\n' + eventData;
        });
...
function MySchedulePage() {
    ...
    const handleExport = ()=>{
        ...
        const icsContent = generateIcsFile(upcomingAssignments);
        const blob = new Blob([
            icsContent
        ], {
...
//# sourceMappingURL=src_8f959c7d._.js.map